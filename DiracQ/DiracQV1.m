...
(* Only launch palette when a frontend is available (not in wolframscript/batch mode) *)
If[$FrontEnd =!= Null,
	DiracQPalette,
	(* Initialize operator lists with all built-in operators for headless/script usage *)
	Operators = {b, b\[Dagger], f, f\[Dagger], J, X, \[Sigma], q, p, Bra, Ket};
	SecondaryOperators = {Subscript[n, b], Subscript[n, f], J^Plus, J^Minus, \[Sigma]^Plus, \[Sigma]^Minus, OverVector[q], OverVector[p]};
	ApplyDefinition = True;
	Decomposition = CommutatorRule;
];
